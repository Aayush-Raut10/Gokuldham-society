...
import { fetchData, updateForm } from '@/services/httpMethods'
...
const EditNotice = () => {
    ...
    const {
        ...
        onSubmit: async (formData) => {
            ...
            const res = await updateForm(payload, '/api/notices')
        ...
        resetOnSubmit: true,
        successMessage: 'Notice updated successfully! Redirecting...'
    })
    ...
    useEffect(() => {
        const fetchNoticeData = async () => {
            ...
            if (res.success && res.data) {
                ...
                if (res.data.image) {
                    const response = await fetch(res.data.image)
                    const blob = await response.blob()
                    const file = new File([blob], 'notice-image', { type: blob.type })
